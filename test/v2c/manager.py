...
from snmp.v2c.requests import pduTypes

class SNMPv2cManagerTest(unittest.TestCase):
            ...
            if self.response is not None:
                if len(self.messages) == self.msgCount:
                    ...
                    if self.listener is not None:
                        self.listener.hear(message, None)
    ...
    def setUp(self):
        ...
        self.manager = SNMPv2cManager(self.admin, self.channel, self.community)
    ...
    def test_response_ignored_after_handle_has_expired(self):
        ...
        msg = Message(ProtocolVersion.SNMPv2c, self.community, self.response)
        handle.push(msg)
    ...
    def test_getBulk_sends_GetBulkRequestPDU(self):
        self.channel.scheduleResponse(self.response)

        oid = OID(*self.oid[:-1])
        vblist = self.manager.getBulk(oid, maxRepetitions=1)
        self.assertEqual(len(self.channel.messages), 1)
        self.assertEqual(self.time(), 0.0)
        self.assertEqual(vblist, self.response.variableBindings)

        pdu = self.channel.messages[0].pdu
        self.assertIsInstance(pdu, GetBulkRequestPDU)
        self.assertEqual(len(pdu.variableBindings), 1)
